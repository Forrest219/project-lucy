...
COMPOSE_FILES=("deploy/compose/docker-compose.demo.yml")
FRESH=0
NO_CACHE=0
BACKUP_DIR=""
SKIP_HEALTH=0
...
Options:
  -f, --compose-file FILE   Compose file (repeatable; default: deploy/compose/docker-compose.demo.yml)
...
Examples:
  ...
  bash scripts/upgrade/upgrade-lucy.sh --fresh
EOF
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    -f|--compose-file)
      if [[ ${#COMPOSE_FILES[@]} -eq 1 && "${COMPOSE_FILES[0]}" == "deploy/compose/docker-compose.demo.yml" ]]; then
        COMPOSE_FILES=()
      fi
      COMPOSE_FILES+=("$2")
      shift 2
...
snapshot_preserve_state() {
  ...
  node "${ROOT}/scripts/upgrade/upgrade-lucy-cli.mjs" snapshot \
    --volume "${VOLUME_FULL}" \
...
build_and_up() {
  if printf '%s\n' "${COMPOSE_FILES[@]}" | grep -q 'deploy/compose/docker-compose.demo.yml'; then
    bash "${ROOT}/scripts/demo/sync-demo-skills-template.sh"
  fi
...
if [[ "${FRESH}" -eq 0 && "${SKIP_PRESERVE_CHECK}" -eq 0 && "${VOLUME_KIND}" == "named" && -n "${BEFORE_SNAPSHOT}" ]]; then
  ...
  node "${ROOT}/scripts/upgrade/upgrade-lucy-cli.mjs" verify-snapshot \
