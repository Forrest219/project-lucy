...
#
# Modes:
#   bash scripts/gates/k8s-release-gate.sh                              # H1 static only
#   bash scripts/gates/k8s-release-gate.sh --with-cluster --namespace lucy-test --release lucy-starrocks
#   bash scripts/gates/k8s-release-gate.sh --with-cluster ... --test-upgrade -f examples/values.k3s-test.yaml
#
# Environment:
...
usage() {
  cat <<'EOF'
Usage: bash scripts/gates/k8s-release-gate.sh [options]
...
if [[ "${TEST_UPGRADE}" -eq 1 ]]; then
  ...
  bash "${ROOT}/scripts/gates/k8s-upgrade-gate.sh" "${UPGRADE_ARGS[@]}"
else
...
bash "${ROOT}/scripts/gates/k8s-acceptance.sh" "${ACCEPT_ARGS[@]}"
