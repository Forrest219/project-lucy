...
async function localSmoke() {
  ...
  await run("bash", ["-n", "scripts/runtime/docker-entrypoint.sh", "scripts/runtime/docker-healthcheck.sh"]);
...
async function dockerSmoke() {
  ...
  try {
    ...
    await run("bash", ["scripts/release/assert-image-elf-arch.sh", "project-lucy:local", targetArch], { env: composeEnv });
