...
describe("p1 release readiness aggregate", () => {
  it("lists all P1 gates without executing them", () => {
    const result = spawnSync("node", ["scripts/gates/p1-release-readiness.mjs", "--list"], {
      cwd: process.cwd(),
      encoding: "utf8"
    });
  ...
  it("dry-runs all P1 gates and writes aggregate evidence", () => {
    ...
    const result = spawnSync("node", ["scripts/gates/p1-release-readiness.mjs", "--dry-run", "--out", out], {
      cwd: process.cwd(),
      encoding: "utf8"
    });
    assert.equal(result.status, 0, result.stderr);
