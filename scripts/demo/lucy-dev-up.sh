#!/usr/bin/env bash
# scripts/demo/lucy-dev-up.sh — auto-detect host arch and start Lucy for dev.
#
# Why this exists:
...
#
# Usage:
#   scripts/demo/lucy-dev-up.sh                  # build + up (default: customer compose)
#   scripts/demo/lucy-dev-up.sh up               # up only (no rebuild)
#   scripts/demo/lucy-dev-up.sh down             # down (keep volume)
#   scripts/demo/lucy-dev-up.sh down -v          # down + delete lucy-data volume
#   scripts/demo/lucy-dev-up.sh logs -f lucy     # tail logs
#   scripts/demo/lucy-dev-up.sh ps               # compose ps
#   scripts/demo/lucy-dev-up.sh exec lucy bash   # shell into running lucy
#
# Flags (passed through to docker compose):
#   -f, --file FILE   Override compose file (default: docker-compose.yml)
#
# Env overrides (rarely needed):
#   TARGETPLATFORM=linux/amd64  scripts/demo/lucy-dev-up.sh     # force amd64 (slow on arm64)
#   TARGETARCH=amd64            scripts/demo/lucy-dev-up.sh
#   UV_INDEX_URL=https://mirrors.aliyun.com/pypi/simple/ \
#     scripts/demo/lucy-dev-up.sh up -d --build                  # PyPI mirror if pypi is blocked
#
# Notes:
...
#     use scripts/upgrade/upgrade-lucy.sh instead.
set -Eeuo pipefail
...
Common invocations:
  ...
  $(basename "$0") -f deploy/compose/docker-compose.demo.yml up -d --build
EOF
}
