...
async function main() {
  if (hasFlag("--help") || hasFlag("-h")) {
    console.log(`Usage:
  node scripts/smoke/p1-observability-smoke.mjs
  node scripts/smoke/p1-observability-smoke.mjs --url http://127.0.0.1:5174/api/observability --out inbox/p1-observability-evidence.json
...
If the WebUI service is not running, the evidence status is "blocked".`);
  ...
  const evidence = {
    ...
    generatedBy: "scripts/smoke/p1-observability-smoke.mjs",
    url,
    httpStatus,
    summary,
    results,
