...
function runReadinessStrict(env) {
  const result = spawnSync(process.execPath, ["scripts/smoke/lucy-r1-readiness.mjs", "--strict", "--target", TARGET_PROFILE.id], {
    cwd: process.cwd(),
    env: { ...process.env, ...env },
    encoding: "utf8"
  });
...
function validateHermesQaSnapshot(hermesQa, errors) {
  ...
  if (generatedBy !== "scripts/smoke/lucy-r1-hermes-report.mjs") errors.push('data.hermesQa.generatedBy must be "scripts/smoke/lucy-r1-hermes-report.mjs"');
...
async function main() {
  ...
  if (observability.status === "skipped") {
    const manifest = {
      ok: false,
      generatedAt: new Date().toISOString(),
      generatedBy: "scripts/smoke/lucy-r1-release-bundle.mjs",
      target: TARGET_PROFILE.id,
