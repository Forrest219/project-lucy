...
const TARGET_PROFILES = {
  doris: {
    ...
    engine: "doris",
    generatedBy: "scripts/smoke/lucy-r1-doris-smoke.mjs"
  },
  starrocks: {
    id: "starrocks",
    label: "StarRocks",
    ...
    engine: "starrocks",
    generatedBy: "scripts/smoke/lucy-r1-doris-smoke.mjs --engine starrocks"
  }
};
...
function checkDoc() {
  if (!requireFile("docs/specs/lucy-r1-controlled-data-service-plan.md", "doc.exists")) return;
  const doc = read("docs/specs/lucy-r1-controlled-data-service-plan.md");
  for (const phrase of [
  ...
  if (!requireFile("docs/runbooks/lucy-r1-release-runbook.md", "runbook.exists")) return;
  const runbook = read("docs/runbooks/lucy-r1-release-runbook.md");
  for (const phrase of [
...
function checkEvidenceGenerators() {
  if (requireFile("scripts/smoke/lucy-r1-doris-smoke.mjs", "doris_smoke.exists")) {
    const script = read("scripts/smoke/lucy-r1-doris-smoke.mjs");
    for (const phrase of [
  ...
  if (requireFile("scripts/smoke/lucy-r1-doris-smoke.test.mjs", "doris_smoke_test.exists")) {
    const test = read("scripts/smoke/lucy-r1-doris-smoke.test.mjs");
  ...
  if (requireFile("scripts/smoke/lucy-r1-hermes-report.mjs", "hermes_report.exists")) {
    const script = read("scripts/smoke/lucy-r1-hermes-report.mjs");
    for (const phrase of [
      "coreMetricQuestions",
      "securityQuestions",
      "minQuestions",
  ...
  requireFile("scripts/smoke/lucy-r1-hermes-report.test.mjs", "hermes_report_test.exists");
  if (requireFile("scripts/smoke/lucy-r1-local-gates.mjs", "local_gates.exists")) {
    const script = read("scripts/smoke/lucy-r1-local-gates.mjs");
    for (const phrase of [
      "r1-readiness",
      "spec-lint",
      "security-baseline",
  ...
  if (requireFile("scripts/smoke/lucy-r1-local-gates.test.mjs", "local_gates_test.exists")) {
    const test = read("scripts/smoke/lucy-r1-local-gates.test.mjs");
  ...
  if (requireFile("scripts/smoke/lucy-r1-mcp-contract-smoke.mjs", "mcp_contract_smoke.exists")) {
    const script = read("scripts/smoke/lucy-r1-mcp-contract-smoke.mjs");
    for (const phrase of [
      "lucy-r1-controlled-data-service",
      "toolSurface",
  ...
  requireFile("scripts/smoke/lucy-r1-mcp-contract-smoke.test.mjs", "mcp_contract_smoke_test.exists");
  if (requireFile("scripts/smoke/lucy-r1-readiness.test.mjs", "readiness_test.exists")) {
    const test = read("scripts/smoke/lucy-r1-readiness.test.mjs");
  ...
  if (requireFile("scripts/smoke/lucy-r1-release-bundle.mjs", "release_bundle.exists")) {
    const script = read("scripts/smoke/lucy-r1-release-bundle.mjs");
    for (const phrase of [
      ...
      "data.hermesQa.generatedBy must be \"scripts/smoke/lucy-r1-hermes-report.mjs\"",
  ...
  if (requireFile("scripts/smoke/lucy-r1-release-bundle.test.mjs", "release_bundle_test.exists")) {
    const test = read("scripts/smoke/lucy-r1-release-bundle.test.mjs");
    ...
    add(test.includes("observability Hermes generatedBy is not the R1 report generator") && (test.includes("data.hermesQa.generatedBy must be \"scripts/smoke/lucy-r1-hermes-report.mjs\"") || test.includes("data.hermesQa.generatedBy must be \"scripts\\/smoke\\/lucy-r1-hermes-report.mjs\"")) ? "pass" : "fail", "release_bundle_test.hermes_generated_by_gate", "Release bundle test rejects observability snapshots not produced by the R1 Hermes report generator");
  ...
  if (requireFile("package.json", "package_json.exists")) {
    ...
    add(scripts["r1:doris-smoke"] === "node scripts/smoke/lucy-r1-doris-smoke.mjs" ? "pass" : "fail", "package.r1_doris_smoke", "package.json exposes r1:doris-smoke");
    add(scripts["r1:doris-smoke:test"] === "node --test scripts/smoke/lucy-r1-doris-smoke.test.mjs" ? "pass" : "fail", "package.r1_doris_smoke_test", "package.json exposes r1:doris-smoke:test");
    add(scripts["r1:hermes-report"] === "node scripts/smoke/lucy-r1-hermes-report.mjs" ? "pass" : "fail", "package.r1_hermes_report", "package.json exposes r1:hermes-report");
    add(scripts["r1:hermes-report:test"] === "node --test scripts/smoke/lucy-r1-hermes-report.test.mjs" ? "pass" : "fail", "package.r1_hermes_report_test", "package.json exposes r1:hermes-report:test");
    add(scripts["r1:local-gates"] === "node scripts/smoke/lucy-r1-local-gates.mjs" ? "pass" : "fail", "package.r1_local_gates", "package.json exposes r1:local-gates");
    add(scripts["r1:local-gates:test"] === "node --test scripts/smoke/lucy-r1-local-gates.test.mjs" ? "pass" : "fail", "package.r1_local_gates_test", "package.json exposes r1:local-gates:test");
    add(scripts["r1:mcp-contract"] === "node scripts/smoke/lucy-r1-mcp-contract-smoke.mjs" ? "pass" : "fail", "package.r1_mcp_contract", "package.json exposes r1:mcp-contract");
    add(scripts["r1:mcp-contract:test"] === "node --test scripts/smoke/lucy-r1-mcp-contract-smoke.test.mjs" ? "pass" : "fail", "package.r1_mcp_contract_test", "package.json exposes r1:mcp-contract:test");
    add(scripts["r1:release-bundle"] === "node scripts/smoke/lucy-r1-release-bundle.mjs" ? "pass" : "fail", "package.r1_release_bundle", "package.json exposes r1:release-bundle");
    add(scripts["r1:release-bundle:test"] === "node --test scripts/smoke/lucy-r1-release-bundle.test.mjs" ? "pass" : "fail", "package.r1_release_bundle_test", "package.json exposes r1:release-bundle:test");
    add(scripts["r1:status"] === "node scripts/smoke/lucy-r1-status.mjs" ? "pass" : "fail", "package.r1_status", "package.json exposes r1:status");
    add(scripts["r1:status:test"] === "node --test scripts/smoke/lucy-r1-status.test.mjs" ? "pass" : "fail", "package.r1_status_test", "package.json exposes r1:status:test");
  }
}
...
function checkObservabilityContract() {
  ...
  if (requireFile("webui/docs/03-api-spec.md", "api_spec.exists")) {
    ...
    add(apiSpec.includes("GET    /api/r1/observability")
      ...
      && apiSpec.includes('"generatedBy": "scripts/smoke/lucy-r1-hermes-report.mjs"')
...
function validateHermesEvidence(filePath) {
  ...
  if (value.generatedBy !== "scripts/smoke/lucy-r1-hermes-report.mjs") errors.push('generatedBy must be "scripts/smoke/lucy-r1-hermes-report.mjs"');
  requireIsoTimestamp(value, "checkedAt", errors);
...
function validateMcpContractEvidence(filePath) {
  ...
  if (value.target !== "lucy-mcp-proxy") errors.push('target must be "lucy-mcp-proxy"');
  if (value.generatedBy !== "scripts/smoke/lucy-r1-mcp-contract-smoke.mjs") errors.push('generatedBy must be "scripts/smoke/lucy-r1-mcp-contract-smoke.mjs"');
