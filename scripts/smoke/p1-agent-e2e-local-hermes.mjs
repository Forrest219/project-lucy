...
const USAGE = `Usage:
  ...
  node scripts/smoke/p1-agent-e2e-local-hermes.mjs --replace-existing
...
function buildTemporaryAccessConfig(rawYaml, { hermesHash, mozHash, created = new Date().toISOString().slice(0, 10) }) {
  ...
  upsertRuntimeUser(config, {
    ...
    name: "Hermes Moz",
    enabled: true,
    note: "Local runtime-only Hermes moz profile E2E token; generated by scripts/smoke/p1-agent-e2e-local-hermes.mjs.",
...
async function runLocalHermesE2E({ args, env = process.env } = {}) {
  ...
  const wrapper = {
    ...
    generatedBy: "scripts/smoke/p1-agent-e2e-local-hermes.mjs",
  ...
  try {
    ...
    const result = await runCapture(process.execPath, [
      "scripts/smoke/p1-agent-e2e.mjs",
      "--profile", "hermes",
      "--profile", "moz",
