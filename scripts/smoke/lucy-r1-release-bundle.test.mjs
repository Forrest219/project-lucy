...
const SCRIPT = path.join(ROOT, "scripts", "smoke", "lucy-r1-release-bundle.mjs");
...
const validMcpContract = {
  ...
  generatedBy: "scripts/smoke/lucy-r1-mcp-contract-smoke.mjs",
...
test("release bundle fails closed when observability Hermes generatedBy is not the R1 report generator", async () => {
  await withTempDir(async (dir) => {
    ...
    assert.equal(manifest.ok, false);
    assert.match(manifest.reason, /observability snapshot failed R1 validation/);
    assert.match(manifest.reason, /data.hermesQa.generatedBy must be "scripts\/smoke\/lucy-r1-hermes-report.mjs"/);
