...
function cliReadyRun({ runnerStdout } = {}) {
  return async (command, args) => {
    ...
    if (command === 'node' && args[0] === 'scripts/eval/eval-runner.mjs') {
      return {
        code: 0,
...
test('parseArgs supports suite selection and explicit MCP token requirement', () => {
  const args = parseArgs([
    'node',
    'scripts/smoke/p1-business-eval-full.mjs',
    '--suite',
    'superstore',
    '--suite',
    'data_agent_poc',
...
test('runFullEval returns blocked evidence without running eval-runner', async () => {
  ...
  const run = async (command, args) => {
    if (command === 'node' && args[0] === 'scripts/eval/eval-runner.mjs') runnerCalled = true;
