...
const USAGE = `Usage: node scripts/smoke/p1-business-eval-full.mjs [options]
...
Options:
  ...
  --retries <n>            Pass through to scripts/eval/eval-runner.mjs --retries
...
function evidenceBase({ status, exitCode, args, suites, precheck }) {
  return {
    ...
    status,
    exitCode,
    command: 'scripts/smoke/p1-business-eval-full.mjs',
    config: {
