...
if (help) {
  console.log(`Usage:
  node scripts/smoke/fast-smoke.mjs
  node scripts/smoke/fast-smoke.mjs --out inbox/fast-smoke-evidence.json
  node scripts/smoke/fast-smoke.mjs --skip-ui
...
const evidence = {
  ...
  version: "2.0.0",
  generatedBy: "scripts/smoke/fast-smoke.mjs",
...
async function main() {
  ...
  try {
    ...
    await runCommand("node", ["scripts/smoke/p1-skills-smoke.mjs"]);
    await runCommand("node", ["scripts/smoke/headless-config-smoke.mjs"]);
    await runCommand("node", ["scripts/release/verify-license-baseline.mjs"]);
