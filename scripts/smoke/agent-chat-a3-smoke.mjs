...
export function probeHermesCallableTools({ root = ROOT, envFile = "agent-chat/.env", spawnImpl = spawnSync } = {}) {
  ...
  const result = spawnImpl("docker", [
    "compose", "-f", "deploy/compose/docker-compose.agent-chat.yml", "--profile", "agent-chat", "--env-file", envFile,
...
export async function runStaticChecks(evidence, { root = ROOT } = {}) {
  ...
  ok = record(evidence, "composeOverlayExists", await ex("deploy/compose/docker-compose.agent-chat.yml")) && ok;
  ...
  ok = record(evidence, "runbookExists", await ex("docs/runbooks/runbook-lucy-agent-chat-a3.md")) && ok;
  ok = record(evidence, "designSpecExists", await ex("docs/design/design-lucy-agent-chat-a3.md")) && ok;

  if (await ex("deploy/compose/docker-compose.agent-chat.yml")) {
    const compose = await read("deploy/compose/docker-compose.agent-chat.yml");
...
Does NOT belong in smoke:p0:headless-config.`);
  ...
  const evidence = {
    ...
    generatedBy: "scripts/smoke/agent-chat-a3-smoke.mjs",
    live,
    status: "fail",
    checks: {},
    notes: []
