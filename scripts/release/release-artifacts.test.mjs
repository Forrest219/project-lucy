...
describe("release-artifacts A3 customer-bundle hygiene", () => {
  it("strips commands whose implementation is absent from the source bundle", () => {
    const input = {
      ...
      scripts: {
        "smoke:p0": "node a",
        "smoke:p0:delivery-isolation": "node scripts/smoke/lucy-delivery-isolation-smoke.mjs",
        "smoke:p0:delivery-isolation:test": "node --test scripts/smoke/lucy-delivery-isolation-smoke.test.mjs",
    ...
    assert.equal(out.scripts["smoke:p0:delivery-isolation"], "node scripts/smoke/lucy-delivery-isolation-smoke.mjs");
  ...
  it("createSourceBundle strips A3 scripts in tar only; repo root package.json unchanged", async () => {
    ...
    try {
      ...
      assert.match(listing.stdout, /scripts\/smoke\/lucy-delivery-isolation-smoke\.mjs/);
      ...
      assert.equal(
        ...
        "node scripts/smoke/lucy-delivery-isolation-smoke.mjs"
      );
      ...
      const layers = await readFile(path.join(ROOT, "docs/governance/test-layers-and-release-gates.md"), "utf8");
      const deploy = await readFile(path.join(ROOT, "docs/runbooks/deployment-docker.md"), "utf8");
