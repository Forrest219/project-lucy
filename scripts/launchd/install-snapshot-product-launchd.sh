...
#
# 关联文档：docs/webui/webui-snapshot-product.md §7（调度接口契约）、§11 验收 #10
#
...
#
# 显式注册（用户明确要求执行安装时）：
#   bash scripts/launchd/install-snapshot-product-launchd.sh --bootstrap
#   这条命令会先 bootout（如果旧任务在）再 bootstrap（注册到 launchd gui session）。
#
# 卸载：
#   bash scripts/launchd/install-snapshot-product-launchd.sh --uninstall
#   会 bootout 并删除 plist。
#
# 环境变量（可覆盖默认）：
...
if [[ ! -f "$PLIST_TEMPLATE" ]]; then
  ...
  echo "  Expected to be the reviewer-auditable plist (see docs/webui/webui-snapshot-product.md §11 #10)." >&2
  exit 1
fi
...
usage() {
  cat <<USAGE
用法：bash scripts/launchd/install-snapshot-product-launchd.sh [--bootstrap|--uninstall|--help]
...
case "${1:-}" in
  "")
    ...
    echo "  bash scripts/launchd/install-snapshot-product-launchd.sh --bootstrap"
