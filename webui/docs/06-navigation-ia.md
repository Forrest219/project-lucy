...
- `Catalog` 像数据表列表，但没有明确告诉用户这是“业务上下文”的表目录。
...
2. 页面标题必须说明当前维护的是“数据接入”“业务上下文”“语义发布”“质量评测”还是“访问治理”。
...
| 分组 | 导航项 | 路由 | 说明 |
| --- | --- | --- | --- |
| 运行状态 | 系统概览 | `/overview` | 确认系统可用，处理当前待办（Spec 142） |
| 运行状态 | 调用监控 | `/ops/calls` | 准实时查看 MCP 工具调用量、成败与请求时效（Spec 143） |
...
| 业务上下文 | 业务 Skill | `/skills` | 查看受治理的业务 Skill 资产及其发布状态与角色授权（Spec 144） |
...
| 连通测试 | `数据接入 / 连通测试` |
| 语义资产 | `业务上下文 / 语义资产` |
| 表语义编辑 | `业务上下文 / <schema> / <table>` |
| 关联关系 | `业务上下文 / 关联关系 / <table>` |
| 业务 Wiki | `业务上下文 / 业务 Wiki` |
| 业务 Skill | `业务上下文 / 业务 Skill` |
| 发布工作台 | `语义发布 / 发布工作台` |
| 发布记录 | `语义发布 / 发布记录` |
| 评测用例 | `质量评测 / 评测用例` |
| 运行历史 | `质量评测 / 运行历史` |
...
- 普通用户从左侧导航能识别 6+1 结构：运行状态、数据接入、业务上下文、语义发布、质量评测、访问治理、系统设置（Spec 143：原置顶「系统概览」单项并入「运行状态」分组；Spec 142：系统概览首页聚焦精简；Spec 144：「业务 Skill」入业务上下文组，「语义建模」保留为建模活动概念词）。
- 任一上下文页面能通过面包屑看出当前位置。
