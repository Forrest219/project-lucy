...
| WebUI Admin | 管理员 | WebUI 登录账户 | 超管（除非专指所有者）、用户（易与 Agent 混淆） | WO-62：本地登录主体，非 MCP Agent |
| WebUI Admin Owner | 所有者 | 所有者管理员 | 超管（主术语） | 可管理其他管理员账户 |
| WebUI Login | 登录 | 管理员登录 | Sign in 裸用 | `/login` |
