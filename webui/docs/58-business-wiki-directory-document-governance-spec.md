...
- 不实现批量删除、批量移动、目录递归移动或目录重命名。（目录重命名已独立为 Spec 109 / `UX-WIKI-044`；**单文档删除**已独立为 Spec 118 / `UX-WIKI-045`，均不在本 Spec 范围。）
- 不实现软删除回收站；删除保护先通过确认、阻止非空目录和版本记录降低风险。（单文档硬删除的确认与配置审计见 Spec 118。）
