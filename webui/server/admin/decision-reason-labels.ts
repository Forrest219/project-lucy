/** Spec 99 — decision_reason code → Chinese primary label. */
...
export function decisionReasonLabel(code: string | null | undefined): string {
  ...
  if (code.startsWith("role_not_found:")) return "Role 配置异常";
  if (code.startsWith("role_resolution_failed:")) return "Role 配置异常";
  if (code === "user_disabled" || code === "agent_disabled") return "Agent 已禁用";
  if (code === "token_revoked") return "Token 不可用";
  if (code === "token_expired") return "Token 不可用";
