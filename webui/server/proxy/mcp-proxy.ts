...
function lucyExplainQueryTool() {
  return {
    ...
    description: "Diagnostic tool only: local permission / ForcedPredicateAST explanation without upstream execution or result rows. Use after a query is denied or when debugging guardrails; not a data-return path. dimensions/order_by must be arrays of objects, never strings.",
    inputSchema: {
      type: "object",
      properties: semanticQueryInputProperties(),
      required: ["connectionId"],
...
function lucyQueryUpstreamArgs(args: unknown): Record<string, unknown> {
  const record = args && typeof args === "object" && !Array.isArray(args) ? args as Record<string, unknown> : {};
  // Strip camelCase user alias; snake_case forced_filters is only set by Proxy after ACL.
  const { forcedFilters: _userForcedFilters, ...rest } = record;
  return {
    ...rest,
    filters: normalizeLucyFiltersForUpstream(rest.filters),
    limit: numericLimit(rest.limit)
  };
}
...
async function lucyExplainQuery(identity: Identity, args: unknown): Promise<Record<string, unknown>> {
  ...
  // When measures imply sources but extractSourceRefs is empty, derive from capability tables via measures.
