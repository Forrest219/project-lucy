...
| 撰写人 | Claude |
| 委托人 | 待确认 |
| 基于材料 | AGENTS.md、docs/governance/DEVELOPMENT.md、docs/governance/project-overview.md、ktx.yaml、.mcp.json、docs/specs/lucy-skills-mcp-spec.md、skills/、evals/ |
...
| **开发态**：改代码 / 改配置 | [`AGENTS.md`](AGENTS.md) → [`docs/governance/DEVELOPMENT.md`](docs/governance/DEVELOPMENT.md) | Claude Code、Codex 等 coding agent | agent 启动时读取 AGENTS.md |
...
| `docs/agent-pipeline.md` | 可选的完整交付流水线（pm / architect / coder / tester / reviewer 5 角色）；仅在需要正式审计、不可逆操作或团队交接时启用。 |
| `docs/specs/lucy-skills-mcp-spec.md` | 规划中（P1.5）的独立 MCP server spec：将 `skills/` 以标准 MCP Resource 形式暴露给 data agent，尚未实现。 |
...
**客户部署 / 运维**：见 [`docs/runbooks/customer-deployment-guide.md`](docs/runbooks/customer-deployment-guide.md)、[`docs/runbooks/deployment-docker.md`](docs/runbooks/deployment-docker.md) 与 [`docs/runbooks/admin-guide.md`](docs/runbooks/admin-guide.md)。首版客户标准路径是标准 Lucy image + `customer-config/` 配置包 bind mount 到 `/data/lucy`；持续维护表语义、wiki、eval 和权限时先改 `customer-config/`，再运行 reindex/validate/eval gate。
**全链路测试用例**：见 [`docs/governance/lucy-test-cases.md`](docs/governance/lucy-test-cases.md)。

**首次 Onboarding**（详见 `docs/governance/DEVELOPMENT.md §Onboarding`）：
...
本地 Docker 验证用 `docker compose -f deploy/compose/docker-compose.demo.yml up`（仅 `demo-mysql`）。客户交付默认 seed 来自 `customer-config.example`，与开发者私有测试库分离。
...
spec 见 `docs/webui/webui-snapshot-product.md`。截图脚本是只读工具，不改任何业务文件，输出落到 `var/screenshots/` 与 `var/logs/`（已在 `.gitignore`）。
...
bash scripts/launchd/install-snapshot-product-launchd.sh              # 写 ~/Library/LaunchAgents/com.lucy.snapshot-product.plist
bash scripts/launchd/install-snapshot-product-launchd.sh --bootstrap   # 用户显式要求时才跑：launchctl bootstrap
bash scripts/launchd/install-snapshot-product-launchd.sh --uninstall   # 卸载
