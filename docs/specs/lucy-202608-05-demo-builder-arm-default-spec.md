...
> **Dockerfile 契约更新（2026-08-05）**：本规格 v0.2 曾写「不修改 `FROM --platform=$BUILDPLATFORM`」。该限制已被 [`docs/specs/lucy-202608-08-image-arch-and-ktx-baseline-fix.md`](./lucy-202608-08-image-arch-and-ktx-baseline-fix.md) **废止并替换**——`Dockerfile` 现绑定 `TARGETPLATFORM`。本规格其余条款（`BUILDX_BUILDER=default`、禁止客户 `create --use`）仍然有效；平台变量名以 202608-08 为准（`TARGETPLATFORM` / `TARGETARCH`）。
...
`deploy/compose/docker-compose.demo.yml` 已具备 `BUILDPLATFORM`/`TARGETARCH` 的 arm64 默认值；仅靠 compose args **不足以**阻止错选 builder。Compose yaml 无法可靠写死 `build.builder`，必须用 `BUILDX_BUILDER=default`（或等价 CLI）锁定本次构建。

### 1.2 目标

- 本机执行 **推荐入口** `npm run demo:rebuild`（`scripts/demo/rebuild-demo-lucy.sh`）时：固定 `BUILDX_BUILDER=default`，按 host arch 设置 `BUILDPLATFORM`/`TARGETARCH`，一次完成重建，镜像平台与 host 一致。
...
### 2.1 推荐入口：`scripts/demo/rebuild-demo-lucy.sh`
...
| Compose | `docker compose -f deploy/compose/docker-compose.demo.yml up -d --build lucy`；`--no-cache` 时改为 `build --no-cache` + `up -d --force-recreate --no-deps` |
| 断言 | 结束后 `project-lucy:demo` 的 `Os/Architecture` 必须等于所用 `TARGETPLATFORM`；并跑 `scripts/release/assert-image-elf-arch.sh` |

npm 入口：`package.json` → `"demo:rebuild": "bash scripts/demo/rebuild-demo-lucy.sh"`。

### 2.2 `deploy/compose/docker-compose.demo.yml` / `deploy/compose/docker-compose.postgres-demo.yml`
...
`docs/specs/lucy-customer-amd64-offline-delivery-spec.md` 与对应 plan：
...
2. **拒绝客户 builder（必做）**
   ```bash
   BUILDX_BUILDER=lucy-amd64 bash scripts/demo/rebuild-demo-lucy.sh
...
4. **amd64 覆盖（可选；勿在 ARM 上做 --no-cache DoD）**
   ...
   TARGETPLATFORM=linux/amd64 TARGETARCH=amd64 BUILDX_BUILDER=default \
     docker compose -f deploy/compose/docker-compose.demo.yml build lucy
   ```
...
- [ ] `scripts/demo/rebuild-demo-lucy.sh` + `npm run demo:rebuild` 已落地。
- [ ] `deploy/compose/docker-compose.demo.yml` / `deploy/compose/docker-compose.postgres-demo.yml` 含 `TARGETPLATFORM`/`TARGETARCH` 默认值。
- [ ] 客户交付 spec/plan：**无** `create --use`；有结束后 `docker buildx use default`。
- [ ] `docs/governance/DEVELOPMENT.md` 含本地 Docker demo 重建约定。
...
- **demo 镜像**：`project-lucy:demo`，由 `deploy/compose/docker-compose.demo.yml` / `demo:rebuild` 构建。
