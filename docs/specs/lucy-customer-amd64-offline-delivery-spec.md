...
| Entrypoint | `tini -- /app/scripts/runtime/docker-entrypoint.sh` | `Dockerfile` |
| Healthcheck | `scripts/runtime/docker-healthcheck.sh` (30s interval, 10s timeout, 3 retries, 30s start period) | `Dockerfile` |
...
2. **ELF 二进制门禁（G2，必做）**：`bash scripts/release/assert-image-elf-arch.sh project-lucy:customer-amd64-0.17.0-20260902-b262798 amd64` 必须通过；检查 **`/usr/local/bin/node` 与 `/usr/bin/tini`**。仅检查 metadata **不够**——历史上出现过「元数据 amd64、ELF 实为 aarch64」，客户报 `exec /usr/bin/tini: exec format error`。
...
推荐一键入口：`bash scripts/release/build-customer-amd64-image.sh`（含 G1–G4）。完整清单见 [`docs/runbooks/customer-amd64-image-build-checklist.md`](../runbooks/customer-amd64-image-build-checklist.md)。
...
├── docker-compose.customer-config.yml # 来自仓库 deploy/compose/（打包至包根；bind mount override）
...
│   ├── lucy-deployment-docker.md          # 来自 docs/runbooks/deployment-docker.md
│   ├── lucy-admin-guide.md                # 来自 docs/runbooks/admin-guide.md
│   ├── lucy-security-guide.md             # 来自 docs/runbooks/security-guide.md
...
- **审计落盘路径**：`/data/lucy/.ktx-ui/audit.sqlite`，按 `docs/runbooks/security-guide.md` §4 执行。
...
# 2b. ELF 二进制门禁（必做）
bash scripts/release/assert-image-elf-arch.sh project-lucy:customer-amd64-0.17.0-20260902-b262798 amd64
...
docker compose -f docker-compose.yml -f deploy/compose/docker-compose.customer-config.yml up -d
curl -sf http://localhost:5174/api/health
docker compose -f docker-compose.yml -f deploy/compose/docker-compose.customer-config.yml down
```

## 7. 落位 / 索引

- 规格本体：`docs/specs/lucy-customer-amd64-offline-delivery-spec.md`
- 配套 plan：`docs/plans/wo-202608-07-customer-amd64-delivery.md`
- 架构修复规格：`docs/specs/lucy-202608-08-image-arch-and-ktx-baseline-fix.md`
...
- 邮件草稿涉及的所有术语必须与 `docs/runbooks/customer-deployment-guide.md` 第 1–5 节保持一致（MCP Proxy、customer config package、bind mount、`/data/lucy`）。
