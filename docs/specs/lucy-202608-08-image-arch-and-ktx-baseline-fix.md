...
| 撰写人 | Composer |
| 委托人 | 张兴晨 |
| 基于材料 | 客户部署风险复核；`inbox/customer-amd64-offline-package` 坏镜像取证；`Dockerfile`；`.github/workflows/lucy-release.yml`；`docs/specs/lucy-202608-05-demo-builder-arm-default-spec.md` |
| 适用范围 | Docker 镜像构建契约、客户 amd64 离线交付门禁、Release CI KTX 版本基线 |
| 输出位置 | `docs/specs/lucy-202608-08-image-arch-and-ktx-baseline-fix.md` |
...
| 文件 | 要求 |
|---|---|
| `deploy/compose/docker-compose.demo.yml` | build args：`TARGETPLATFORM`（默认 `linux/arm64`）、`TARGETARCH`（默认 `arm64`）；删除对 BUILDPLATFORM 的 FROM 契约说明 |
| `deploy/compose/docker-compose.postgres-demo.yml` | 同上 |
...
| `scripts/demo/rebuild-demo-lucy.sh` | 按 host 设置 `TARGETPLATFORM`/`TARGETARCH`；断言 `project-lucy:demo` 的 `Os/Architecture` **等于** `TARGETPLATFORM` |
...
更新 `docs/specs/lucy-customer-amd64-offline-delivery-spec.md` 与 `docs/plans/wo-202608-07-customer-amd64-delivery.md`：
...
新增脚本：`scripts/release/assert-image-elf-arch.sh`

```text
用法: scripts/release/assert-image-elf-arch.sh <image> <expected-arch>
...
| 文档 | 变更 |
|---|---|
| `docs/governance/version-matrix.md` | Image architecture evidence 改为 `TARGETPLATFORM` + ELF 门禁 |
| `docs/governance/DEVELOPMENT.md` | demo 重建说明改为 `TARGETPLATFORM`/`TARGETARCH` |
| `docs/specs/lucy-202608-05-demo-builder-arm-default-spec.md` | 增加「Dockerfile 契约已被 202608-08 废止/替换」交叉引用（保留 builder 隔离条款） |
...
3. `bash -n scripts/demo/rebuild-demo-lucy.sh scripts/release/assert-image-elf-arch.sh`
4. 对已知坏镜像（若本地仍 load）：
   ```bash
...
5. 可选（Apple Silicon，有缓存即可）：`npm run demo:rebuild` 后镜像为 `linux/arm64`，且
   `bash scripts/release/assert-image-elf-arch.sh project-lucy:demo arm64` 通过。
