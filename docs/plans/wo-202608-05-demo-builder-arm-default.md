...
> 对应规格：[`docs/specs/lucy-202608-05-demo-builder-arm-default-spec.md`](../specs/lucy-202608-05-demo-builder-arm-default-spec.md) **v0.2**
>
...
grep -nE "BUILDPLATFORM|TARGETARCH" deploy/compose/docker-compose.demo.yml deploy/compose/docker-compose.postgres-demo.yml
test -x scripts/demo/rebuild-demo-lucy.sh || echo "script missing"
```
...
确保 `scripts/demo/rebuild-demo-lucy.sh` 可执行，且 `package.json` 有 `"demo:rebuild"`。

契约见 spec §2.1（强制 `BUILDX_BUILDER=default`、拒绝 `lucy-amd64`/`lucy-builder`、按 host 设 platform、结束后断言镜像 arch）。

### Step 2 — compose args

- `deploy/compose/docker-compose.demo.yml`：保留/确认 `BUILDPLATFORM`/`TARGETARCH` 默认 `linux/arm64` / `arm64`，注释指向脚本。
- `deploy/compose/docker-compose.postgres-demo.yml`：按相同模式补齐两个 args。
...
```bash
test -x scripts/demo/rebuild-demo-lucy.sh
grep -q demo:rebuild package.json
grep -n BUILDPLATFORM deploy/compose/docker-compose.demo.yml deploy/compose/docker-compose.postgres-demo.yml
grep -n "禁止 --use\|禁止加 --use" docs/specs/lucy-customer-amd64-offline-delivery-spec.md
```
...
- [ ] `docs/governance/DEVELOPMENT.md` 已补充重建约定。
