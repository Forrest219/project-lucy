...
这三层不能混用。WebUI 与 Agent 配置只认 **Advertise**：

| 层 | 变量 / 位置 | 默认（customer compose） | 给谁用 |
|---|---|---|---|
| Listen | `LUCY_PROXY_HOST`:`LUCY_PROXY_PORT` | 容器内 `0.0.0.0:7879` | Proxy 进程绑定；**勿**写给 Agent |
| Publish | compose `ports` / Ingress | 宿主 `7879→7879`（可用 `LUCY_PROXY_HOST_PORT` remap） | 网络可达 |
| Advertise | `LUCY_PUBLIC_MCP_URL` | compose 默认 `http://127.0.0.1:7879/mcp`（与默认 Publish 对齐） | WebUI 展示与 Agent 复制的**唯一**事实源 |

规则：

- **直连宿主端口**：remap `LUCY_PROXY_HOST_PORT` 时，**必须同步**改 `LUCY_PUBLIC_MCP_URL`（见仓库根 `.env.example`）。
- **反向代理 / 域名 / Ingress**：只改 Advertise 为对外 URL（如 `https://lucy.example.com/mcp`）；Listen/Publish 可保持内部端口。
- 未设置 `LUCY_PUBLIC_MCP_URL` 时，WebUI 走本地 fallback `http://127.0.0.1:7879/mcp`，状态为 `fallback`，**不计** MCP / 部署就绪（仅本地 npm 开发便利；仍可复制，但不可用于客户交付）。

默认客户入口（Advertise）：

```text
MCP: <LUCY_PUBLIC_MCP_URL>
# customer compose 未覆盖时：http://127.0.0.1:7879/mcp
```

如宿主机端口冲突，宿主映射与 Advertise **一起改**：

```bash
LUCY_WEBUI_HOST_PORT=55175 \
LUCY_PROXY_HOST_PORT=57880 \
LUCY_PUBLIC_MCP_URL=http://127.0.0.1:57880/mcp \
...
不会 seed 仓库根上的内网测试语义层或开发者私有 ACL。本地 demo 请使用 `deploy/compose/docker-compose.demo.yml`。
...
```bash
bash scripts/upgrade/upgrade-lucy.sh -f docker-compose.yml --backup-dir inbox/backups
```
...
```bash
docker compose -f docker-compose.yml -f deploy/compose/docker-compose.customer-config.yml up -d --build
```

`deploy/compose/docker-compose.customer-config.yml` 内容：
...
默认镜像首次启动时会从 **`customer-config.example`** 组装的 template seed 出 `/data/lucy/ktx.yaml`（`customer-db` 占位）。客户生产部署必须编辑 volume（或 bind-mount `customer-config/`）中的连接信息与密码文件路径。`deploy/compose/docker-compose.customer-config.yml` 会设置 `LUCY_DISABLE_TEMPLATE_SYNC=1`，避免 template 再次合并进客户目录。
...
docker compose -f docker-compose.yml -f deploy/compose/docker-compose.secrets.yml up -d
```
...
| `LUCY_PUBLIC_MCP_URL` | compose default `http://127.0.0.1:7879/mcp` (customer) / `http://127.0.0.1:57881/mcp` (demo); unset in bare npm → fallback | **Advertise**：Public MCP endpoint shown by WebUI and copied into agent configs. Distinct from Listen (`LUCY_PROXY_*`) and Publish (host `ports` / Ingress). Remap host proxy port → update this URL too; reverse-proxy deployments set the external https URL instead. Unset → `fallback` (local npm only; not deployment-ready). |
...
docker compose -f deploy/compose/docker-compose.demo.yml ps
curl http://127.0.0.1:55176/api/health

docker compose -f deploy/compose/docker-compose.demo.yml exec demo-db \
...
docker compose -f deploy/compose/docker-compose.demo.yml exec lucy \
...
该自动化 gate 使用 `deploy/compose/docker-compose.demo.yml` 启动 MySQL demo DB 与 Lucy，验证：
...
如果 bind mount 后配置看不到，先确认是否使用了 `deploy/compose/docker-compose.customer-config.yml`，以及 `./customer-config` 是否相对当前 compose 执行目录存在。
...
完整 ~35 条 TC 含命令、参数、错误对照见 `docs/governance/lucy-test-cases.md`，随发布产物输出到 `release/lucy-test-cases.md`。

```bash
npm run release:artifacts -- --out release/
# 或直接调用：
node scripts/release/release-artifacts.mjs --out release/
```
...
POC 演示 **不在** 默认 `docker-compose.yml` 内提供 demo 数据库。完整边界见 `docs/specs/lucy-poc-demo-isolation-spec.md`。

`deploy/compose/docker-compose.demo.yml` 与 `deploy/compose/docker-compose.postgres-demo.yml` 挂载 **demo 专用** eval，与自包含 DB 基线对齐：

| Compose | Eval mount |
|---|---|
| `deploy/compose/docker-compose.demo.yml` | `../../examples/docker-demo/project-template/evals` → `/data/lucy/evals:ro` |
| `deploy/compose/docker-compose.postgres-demo.yml` | `../../examples/postgres-demo/project-template/evals` → `/data/lucy/evals:ro` |
