...
| 输出位置 | `docs/runbooks/customer-amd64-image-build-checklist.md` |
...
```bash
bash scripts/release/build-customer-amd64-image.sh
```
...
| G2 | ELF 门禁 | `bash scripts/release/assert-image-elf-arch.sh <tag> amd64` | **node + tini** 均为 x86-64，不得含 ARM/aarch64 |
...
| G8 | K8s 升级契约（**仅镜像**） | `bash scripts/gates/g8-image-k8s-contract-gate.sh <tag>` | UID 10001；空 volume 自动 `.git`；runtime 在 `/home/lucy/.ktx` |
...
bash scripts/gates/g8-image-k8s-contract-gate.sh "${TAG}"
```
...
- 首次启动若 volume 权限报错：对 named volume 执行一次性 `chown 10001:10001`（见 `docs/runbooks/customer-deployment-guide.md`）
...
一键构建脚本：`bash scripts/release/build-customer-amd64-image.sh`  
...
| **K6** | **包完整性自证** | `bash scripts/gates/verify-k8s-package.sh --tar <pkg>` 或封包脚本自动执行 |
...
- `scripts/release/build-customer-amd64-image.sh`
- `scripts/gates/g8-image-k8s-contract-gate.sh`
- `scripts/gates/verify-k8s-package.sh`
- `scripts/gates/k8s-upgrade-gate.sh`
