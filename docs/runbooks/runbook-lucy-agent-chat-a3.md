...
| 设计事实源 | [`docs/design/design-lucy-agent-chat-a3.md`](../design/design-lucy-agent-chat-a3.md) |
...
2. **默认 compose 不含 Chat**：勿把 `deploy/compose/docker-compose.agent-chat.yml` 并进客户默认 `docker compose up`。
...
```bash
docker compose -f deploy/compose/docker-compose.agent-chat.yml --profile agent-chat --env-file agent-chat/.env down
```
...
3. `docker compose -f deploy/compose/docker-compose.agent-chat.yml --profile agent-chat --env-file agent-chat/.env down -v`
