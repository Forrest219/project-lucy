...
| 撰写人 | Cursor Agent |
| 委托人 | xingchen |
| 基于材料 | 既有架构评估结论（Open WebUI + Hermes API Server → Lucy MCP）；`docs/governance/vision.md`；`docs/runbooks/agent-integration-guide.md`；`docs/specs/lucy-platform-goal-checklist.md`；Hermes API Server / Open WebUI 官方集成文档 |
| 适用范围 | 可选用户侧流式对话端的设计冻结与单租户验证；**不**作为默认客户交付或 Proxy 实现工单 |
| 输出位置 | `docs/design/design-lucy-agent-chat-a3.md` |
...
与现有 Agent 接入一致（见 `docs/runbooks/agent-integration-guide.md`、Admin Token 首秀 Hermes snippet）：
...
| `docs/runbooks/agent-integration-guide.md` | 接入契约事实源；本 Spec 不改写其 endpoint/auth 规则 |
...
| 出口 | 状态 | 说明 |
|---|---|---|
| Compose overlay | **已落地** | [`deploy/compose/docker-compose.agent-chat.yml`](../../deploy/compose/docker-compose.agent-chat.yml) + [`agent-chat/`](../../agent-chat)；`--profile agent-chat`；默认 `docker compose up` 不启动 |
| 短 Runbook | **已落地** | [`docs/runbooks/runbook-lucy-agent-chat-a3.md`](../runbooks/runbook-lucy-agent-chat-a3.md) |
...
- `docs/governance/vision.md` — 产品定位  
- `docs/runbooks/agent-integration-guide.md` — MCP 接入  
- `docs/specs/lucy-platform-goal-checklist.md` — 平台验收边界  
