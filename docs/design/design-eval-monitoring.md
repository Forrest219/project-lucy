...
| 撰写人 | Claude Thinker |
| 委托人 | zhangxingchen |
| 基于材料 | project-lucy/scripts/eval/eval-runner.mjs, project-lucy/evals/superstore/eval/superstore-eval-cases.yaml v1.3, project-lucy/docs/governance/eval-quiz-conventions.md, project-lucy/docs/user-guide/eval-meta-governance-training.html v1.2, project-lucy/webui/docs/01-architecture.md |
| 适用范围 | Builder / Reviewer / Minimax M3 直接进入实现 |
| 输出位置 | project-lucy/docs/design/design-eval-monitoring.md |
...
2. **触发运行**：`node scripts/eval/eval-runner.mjs [--case <id>] [--format md|json]`，需要终端、需要 `claude auth status`、需要 `EVAL_KTX_MCP_URL` 环境变量。
...
- 不重写 eval runner：`scripts/eval/eval-runner.mjs` 保持 CLI 入口，WebUI 通过子进程调用。
...
- [假设：runner 产物 `.ktx-ui/eval/latest.{md,json}` 的 JSON 格式即 `summarize()` 的输出]，从 `scripts/eval/eval-runner.mjs` 末尾 export 与 main 流程推断。Builder 应在 spike 阶段把 `latest.json` 一份样本钉到测试 fixture。
- [假设：所有 eval cases 都遵守 `docs/governance/eval-quiz-conventions.md v1.4` schema]。如有跨版本兼容需求，在 yaml 顶层用 `runner_schema_version` 标记，前端按版本走不同表单。
...
提交后：
- 后端 spawn `node scripts/eval/eval-runner.mjs ...`，把进程 PID + 工单 id 写入 SQLite。
...
直接镜像 `docs/governance/eval-quiz-conventions.md` v1.4 + 现有 yaml v1.3 schema：
...
4. spawn `node scripts/eval/eval-runner.mjs --cases <yamlPath> [--case id...] --format json`，stdout 管道到 `<id>.json`，stderr 管道到 `<id>.log`；
...
```
spawn('node', ['scripts/eval/eval-runner.mjs',
       '--cases', yamlPath,
       '--format', 'json',
...
`scripts/eval/eval-runner.mjs` 仍然是 CLI 一等公民：
- 终端用户可以继续 `node scripts/eval/eval-runner.mjs --case <id>` 跑。
- CI 可以继续 `scripts/eval/eval-runner.mjs --write-latest` 用 `.ktx-ui/eval/latest.{md,json}`。
- WebUI 通过 spawn 调用，**不修改 runner 源码**（除非 builder 在 spike 时发现 stderr 行格式不稳定，再做兼容性微调）。

### 7.2 不动 yaml schema

eval cases yaml 仍然是 `docs/governance/eval-quiz-conventions.md` 定义的 schema。WebUI 表单是对 schema 的可视化映射，所有 yaml 字段在 UI 里都能编辑或至少能"以 raw JSON 兜底"。
...
1. **Case CRUD**：在 `/eval/cases/superstore` 复制 `superstore-discount-001` → 改名 `superstore-discount-001-copy` → 保存 → `git diff evals/superstore/eval/superstore-eval-cases.yaml` 能看到新增段；`node scripts/eval/eval-runner.mjs --list-cases` 能列出新 id。
...
7. **CLI 不退化**：`node scripts/eval/eval-runner.mjs --case superstore-discount-001 --write-latest` 仍工作，产物 `.ktx-ui/eval/latest.{md,json}` 仍按原格式写。
8. **回归**：现有所有页面与测试集仍绿。
