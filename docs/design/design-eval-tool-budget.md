...
| 版本 | v0.1 |
| 撰写日期 | 2026-06-22 |
| 适用范围 | `scripts/eval/eval-runner.mjs`、`evals/*/eval/*-eval-cases.yaml`、WebUI Eval run 展示 |
...
- `scripts/eval/eval-runner.mjs::parseClaudeOutput()` 提取：
  - `toolCalls`: `{ id, name, input }[]`
  - `toolCandidates`: 可从 KTX tool result 中恢复 SQL / result 的候选列表。
- `scripts/eval/eval-runner.mjs::checkToolAssertions()` 支持：
  - `required_tool`
  - `forbidden_tool`
...
在 `scripts/eval/eval-runner.mjs` 增加纯函数：
