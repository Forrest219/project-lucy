...
- 全局侧栏在所有分组同时展开时，中间 nav 必须能完整渲染或独立滚动：父容器（`.pl-sidebar`）固定 `h-screen overflow-hidden` + 中间 `<nav>` 必须 `flex-1 min-h-0 overflow-y-auto`；底部 footer（`.pl-sidebar-footer`）必须 `shrink-0` 固定贴底。否则分组数量增加或子菜单展开后，内容会超出 viewport 被 footer 遮挡，出现"icon 只露顶部 1/3""整组不可见"的截断。浏览器验收时需在 1920×1080 视口下展开所有 5 个分组逐一检查子菜单项可见性。
