...
| Test ID / 角色 | 元素 | 出现位置 | 关键文案约束 |
|---|---|---|---|
| `connection-card-<connId>` | 连接卡片根节点 | ConnectionOverview | — |
| `connection-card-<connId>-badge-readonly` | `预期只读` Badge | ConnectionOverview | 禁止 `Read-only expected` 露出 |
| `connection-card-<connId>-config-source` | `配置：ktx.yaml` 短标签 | ConnectionOverview | Tooltip 文案固定 |
| `connection-card-<connId>-schema-row-<schema>` | Schema 表行 | ConnectionOverview | 状态 `已存在` / `缺失 Manifest` / `解析失败` |
| `connection-card-<connId>-btn-add-schema` | `+ 添加 Schema` 按钮 | ConnectionOverview | 禁止 `添加架构` / `添加模式` |
| `connection-card-<connId>-btn-test-connection` | `测试连接` 按钮 | ConnectionOverview | 禁止 `替代测试` |
| `connection-card-<connId>-btn-reload-catalog` | `刷新本地目录` 按钮 | ConnectionOverview | 禁止 `重新加载资产` / `触发 ingest` |
...
**涉及 Selector**：`workbench-pending-count` / `workbench-validate` / `workbench-reindex` / `workbench-upload-semantic-asset` / `workbench-publish-and-reindex` / `workbench-reindex-result` / `connection-card-mysql-aliyun-btn-add-schema` / `add-schema-drawer-step-1..4` / `catalog-upload-input` / `wiki-tree` / `wiki-mode-read` / `sl-ref-badge-mysql-aliyun-dataforai-superstore_orders` / `wiki-save-preflight-drawer` / `breadcrumb`
...
2. `connection-card-mysql-aliyun-btn-add-schema` → 输入 `finance_mart` → `add-schema-drawer-step-2` 测试连接 → `add-schema-drawer-step-3` 写入 `ktx.yaml` → `add-schema-drawer-step-4` 引导上传 Manifest。
3. Schema 行 `connection-card-mysql-aliyun-schema-row-finance_mart` 出现，状态 `缺失 Manifest`。
