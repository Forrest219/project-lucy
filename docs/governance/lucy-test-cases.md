...
| 撰写人 | Hermes（生成器 + 测试矩阵） |
| 适用范围 | 配套 `docs/runbooks/customer-deployment-guide.md` v0.3 + `docs/runbooks/deployment-docker.md` v0.3；覆盖 headless 配置包、demo 链路、postgres-demo 链路、smoke、proxy 鉴权、业务 evals、失败/边界 |
| 基于材料 | `scripts/smoke/headless-config-smoke.mjs`、`scripts/smoke/p0-demo-docker-smoke.mjs`、`scripts/smoke/p0-postgres-demo-smoke.mjs`、`evals/superstore/eval/superstore-eval-cases.yaml`、`examples/docker-demo/mysql/_baseline.json`、`examples/docker-demo/scripts/gen-demo-data.mjs`、`examples/docker-demo/project-template/semantic-layer/demo-mysql/superstore_orders.yaml` |
| 关联文档 | `docs/runbooks/customer-deployment-guide.md` §5/§13、`docs/runbooks/deployment-docker.md` §5/§13 |
...
- `deploy/compose/docker-compose.customer-config.yml` 将 `./customer-config` 挂载到 `/data/lucy`。
...
- 优先级：P0
- 执行：S
- 命令：`docker compose -f deploy/compose/docker-compose.demo.yml build --no-cache lucy`
- Pass 条件：
  - 退出码 `0`
...
- 优先级：P2
- 执行：S
- 命令：`KTX_VERSION=0.16.0 LUCY_EXPECTED_KTX_VERSION=0.16.0 docker compose -f deploy/compose/docker-compose.demo.yml build lucy`
- Pass 条件：
  - 构建成功
...
- 优先级：P0
- 执行：S
- 命令：`docker compose -f deploy/compose/docker-compose.demo.yml up -d --build`
- Pass 条件：
...
- 优先级：P1
- 执行：S
- 命令：`docker compose -f deploy/compose/docker-compose.demo.yml exec lucy nc -zv 127.0.0.1 7878`
- Pass 条件：连接成功
...
- 优先级：P0
- 执行：S
- 命令：`docker compose -f deploy/compose/docker-compose.demo.yml exec lucy ktx --project-dir /data/lucy connection test demo-mysql`
...
- 优先级：P0
- 执行：S
- 命令：`docker compose -f deploy/compose/docker-compose.demo.yml exec lucy ktx --project-dir /data/lucy sl validate superstore_orders --connection-id demo-mysql`
...
- 优先级：P1
- 执行：S
- 命令：`docker compose -f deploy/compose/docker-compose.demo.yml exec lucy ktx --project-dir /data/lucy sl list`
...
- 优先级：P0
- 执行：S（参考 `scripts/smoke/p0-demo-docker-smoke.mjs` `rpc` 逻辑）
...
- 优先级：P0
- 执行：S
- 命令：`npm run eval`（前提：demo stack 已启动；`deploy/compose/docker-compose.demo.yml` v0.2 起已挂载 `./evals:/data/lucy/evals:ro`，KTX MCP 在容器内可访问 evals）
- Pass 条件：所有 P0 用例通过；非 P0 用例失败需记录但不阻断

> 备注：`deploy/compose/docker-compose.demo.yml` 与 `deploy/compose/docker-compose.postgres-demo.yml` 已挂载 `./evals:/data/lucy/evals:ro`（read-only），demo stack 启动后 KTX MCP 在容器内可直接访问 `evals/superstore/eval/`。无需手动 cp 或修改 entrypoint。
...
| 现有脚本 | 是否覆盖本用例 | 备注 |
|---|---|---|
| `scripts/smoke/p0-demo-docker-smoke.mjs` | 覆盖 TC-START-001 / TC-NET-* / TC-DATA-001..004 / TC-PROXY-001/003..005 / TC-BIZ-004 region baseline | 已绿即可视为 §2 §3 §4 §5 大部分用例通过 |
| `scripts/smoke/p0-postgres-demo-smoke.mjs` | 覆盖 TC-START-002 / postgres 链路 | 与 demo 并行执行 |
| `scripts/smoke/p0-business-eval-smoke.mjs` | 部分覆盖 TC-BIZ-009 | 业务侧 smoke 入口 |
