...
- `deploy/compose/docker-compose.customer-config.yml` 将 `./customer-config` bind mount 到 `/data/lucy`。
...
| Entrypoint syntax | `bash -n scripts/runtime/docker-entrypoint.sh scripts/runtime/docker-healthcheck.sh` exits 0 |
