...
|---|---|---|
| Admin | `docs/runbooks/admin-guide.md` | Deploy, continuously configure semantic-layer/wiki/evals/access, operate, upgrade |
| User | `docs/user-guide.md` | How business users ask governed data questions |
| Agent integrator | `docs/runbooks/agent-integration-guide.md` | MCP endpoint, bearer token, client config |
| Operator | `docs/runbooks/troubleshooting-guide.md` | Common failures and escalation packet |
| Security reviewer | `docs/runbooks/security-guide.md` | Token, ACL, audit, secrets, release gate |

## 2. Deployment And Release

| Guide | Purpose |
|---|---|
| `docs/runbooks/customer-deployment-guide.md` | End-to-end customer deployment, headless config package, operations |
...
| `docs/runbooks/customer-k8s-deployer-quickstart.md` | Minimal Helm installer guide for single-replica K8s deployment, Doris validation, persistence, and rollback |
...
| `docs/runbooks/deployment-docker.md` | Docker Compose details and `customer-config/` bind mount |
| `docs/governance/release-ci.md` | Release CI and KTX upgrade gates |
| `docs/governance/version-matrix.md` | Supported runtime and compatibility matrix |
| `docs/governance/test-layers-and-release-gates.md` | Runtime/platform/business eval gate split |

## 3. WebUI Docs

| Guide | Purpose |
|---|---|
| `docs/webui/webui-impl-status.md` | Repository quality/productization reference; not a customer headless entry |
| `docs/webui/webui-module-guide.md` | Optional WebUI module reference for later productization |
