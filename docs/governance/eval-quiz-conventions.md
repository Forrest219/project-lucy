...
| 撰写人 | Claude / Codex |
| 委托人 | project-lucy 团队 |
| 基于材料 | superstore-eval-cases.yaml (v1.3, 17 cases) / superstore-quiz-cases.html (v1.1, 30 题) / docs/governance/DEVELOPMENT.md / 2026-06-18 Runner 可执行性评审意见 / 2026-06-19 安全边界与 CI 退出码评审意见 |
| 适用范围 | 项目内任何为数据集设计 eval cases（YAML，agent 测）和 quiz HTML（人类测）的 agent 必读 |
| 输出位置 | docs/governance/eval-quiz-conventions.md |
...
| Quiz cases | YAML（嵌入 eval cases）→ HTML（渲染生成） | `evals/{domain}/eval/{domain}-eval-cases.yaml`（quiz_cases section）；渲染：`node scripts/eval/render-quiz.mjs` | 人类（业务 / 培训）| 培训 / 验收 |
...
`scripts/smoke/p1-business-eval-full.mjs` 是 P1 完整业务 LLM / agent eval 的编排入口，覆盖 `superstore`、`kx_financial`、`data_agent_poc` 三套 eval YAML。它不替代 `scripts/eval/eval-runner.mjs` 的 case 执行逻辑，只负责前置可用性检查、逐套调用 runner、汇总证据。
...
Runner 输出的 per-case artifact 见 `docs/specs/ceo-one-report-sow-eval-uat-contract.md`。
...
| `docs/governance/DEVELOPMENT.md` | "Spec 落位规则" 章节下应交叉引用本约定 |
