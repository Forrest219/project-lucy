...
| 文档类型 | Design（权限模型方案） |
| 版本 | v1.1 |
| 撰写日期 | 2026-06-22；v1.1 更新 2026-06-22 |
...
| 落位 | `inbox/`（过程产物；P0 闭环后迁 `docs/access-governance-design.md`） |
...
| `?reload=true` 后端未实装 | 撤销延迟 | PATCH enabled=false / token revoke 不能立刻生效 | 本方案 §4 P1 项 |
...
| 4 | `?reload=true` 端点实施：`POST /api/admin/_reload` → proxy 重读 access.yaml 并清缓存；PATCH `enabled:false` / token revoke 默认触发 | `webui/server/admin/reload.ts`、`webui/server/proxy/identity.ts` | 撤销后 ≤2s 旧 token 401；需 A4 产品确认 | 设计 v1.2 §5.3 |
...
| Token 过期 `expires_at` 是否 UI 暴露（A6） | 类型已支持；NewToken 当前未显示日期选择器 | 产品确认是否需要 UI |
| `?reload=true` 主动 reload proxy 必要性（A4） | 当前 30s TTL + 文案告警 | 安全/产品决策；默认接受 TTL |
