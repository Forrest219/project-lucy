# Lucy CFO GTM 差距分析（一次性）

> **落位：** `inbox/` 临时分析，进程结束后可删。  
> **不是** Lucy 产品化权威源。产品化仍以 `docs/lucy-platform-goal-checklist.md`、`docs/lucy-r1-controlled-data-service-plan.md`、`docs/customer-deployment-guide.md` 为准。  
> 本文是 GTM / 采购叙事备忘，不进入 `docs/` 产品文档树。

| 元数据 | 内容 |
|---|---|
| 文档名称 | Lucy CFO GTM 差距分析 |
| 文档类型 | One-time analysis（inbox tmp） |
| 版本 | v0.3 |
| 撰写日期 | 2026-08-28 |
| 委托人 | xingchen |
| 适用范围 | 一次性 GTM 分析：CFO「风险可控」叙事 vs WebUI/POC 资产；不作为产品化 roadmap |
...
本文档将差距量化为可讨论的 backlog 草稿，不含 demo script。**不替代** platform goal checklist 的产品化验收。
...
> **命名约定：** 本文按 **场景域 / 表 / 治理主题** 组织。`wiki/global/poc-*.md` 中的人名是历史 POC 素材，不作为研发分工。
...
| v0.2 | 2026-08-28 | 去除人名绑定；场景改为场景域 A/B/C；P0-4 改为客户 UAT 证据 |
| v0.3 | 2026-08-28 | 从 `docs/` 移出：确认为 inbox 一次性 GTM 分析，非产品化文档 |
